...
var versionCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		fmt.Printf("kuber v%s\nCopyright © 2022 - %d Andrei-Lucian Mociu\n", system.Version, time.Now().Year())
	},
}
...
func init() {
	...
	rootCommand.PersistentFlags().BoolVar(&debug, "debug", false, "pass in order to run kuber in debug mode")
	...
	rootCommand.Flags().Bool("auto-tls", false, "pass in order to have kuber generate and manage its own SSL certificates using Let's Encrypt")
...
func rootCmdRun(cmd *cobra.Command, _ []string) {
	...
	log.WithField("timezone", config.Get().System.Timezone).Info("configured kuber with system timezone")
	if err := config.ConfigureDirectories(); err != nil {
		log.WithField("error", err).Fatal("failed to configure system directories for kubectyl")
		return
	}
	if err := environment.CreateSftpConfigmap(); err != nil {
		log.WithField("error", err).Fatal("failed to create sftp configmap")
	}
	...
	// seamless hard-reboot process in which kuber will re-sync server states based
	...
	// on Kuber. This allows us to ensure the environment exists, write configurations,
	...
	for _, serv := range manager.All() {
		...
		pool.Submit(func() {
			...
			// Check if the server was previously running. If so, attempt to start the server now so that Kuber
	...
	go func() {
		...
		// currently marked as installing/restoring now that Kuber is restarted.
...
func initLogging() {
	...
	p := filepath.Join(dir, "/kuber.log")
	w, err := logrotate.NewFile(p)
	if err != nil {
		log2.Fatalf("cmd/root: failed to create kuber log: %s", err)
	}
...
Kuber was not able to locate your configuration file, and therefore is not
